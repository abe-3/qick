module fifo_dc_sv #(parameter B = 16, parameter N = 4) (
    ...
    bin2gray_sv #(.B(B = N_LOG2)) wptr_i (
        .din(wptr),
        .dout(wptr_g));
    
    // wptr_g_i: Vector synchronizer (only for gray coded)
    synchronizer_vect_sv #(.N(N = 2), .B(B = N_LOG2)) wptr_g_i (
        .rstn(rd_rstn),
        .clk(rd_clk),
        .data_in(wptr_g),
        .data_out(wptr_gc));
    ...
    gray2bin_sv #(.B(B = N_LOG2)) wptr_gc_i (
        .din(wptr_gc),
        .dout(wptr_c));
    ...
    bin2gray_sv #(.B(B = N_LOG2)) rptr_i (
        .din(rptr),
        .dout(rptr_g));

    // rptr_g_i: Vector synchronizer (only for gray coded)
    synchronizer_vect_sv #(.N(N = 2), .B(B = N_LOG2)) rptr_g_i (
        .rstn(wr_rstn),
        .clk(wr_clk),
        .data_in(rptr_g),
        .data_out(rptr_gc));
    ...
    gray2bin_sv #(.B(B = N_LOG2)) rptr_gc_i (
        .din(rptr_gc),
        .dout(rptr_c));
